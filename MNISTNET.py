...
# Final FC 512*2*2 inputs -> 14 Outputs
...
while (True):
    ...
    count = count + 1
    frame = get_color_depth_image(pipeline)
    ...
    if count % 250 == 0:
        ...
        adaptive_th_inv = cv2.adaptiveThreshold(blurgray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 31,
                                                10)
        ...
        with tf.Session() as sess:
            ...
            if ckpt and ckpt.model_checkpoint_path:
                ...
                for n in range(1, labelnum):
                    ...
                    if (str(ex) == "True"):
                        img = []
                        im = cv2.imread("resize/resize{}.jpg".format(n), 0)
                        im = np.ravel(im)
                        img.append(im)
                        img = np.array(img)
